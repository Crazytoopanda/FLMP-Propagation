...
if __name__ == '__main__':
    ...
    if isReal:
        ...
        artificial_file_name=artificial_file_list[0]
        for file_name in file_name_list:
            onmi_arr=[]
            eq_arr=[]
            for i in range(1):
                ...
                write_path = '../data/' + datatype +'/' + artificial_file_name + '_re_pig_copra0107.txt'
                onmi_val, eq = run(edge_path, real_path, feat_path, write_path, dp_s)
                # onmi_arr.append(onmi_val)
                # eq_arr.append(eq)
                # print('平均值')
                # print(np.mean(eq_arr))
                # print(np.mean(onmi_arr))
                # print('人工数据集{0}的{1}的平均eq={2},平均onmi={3}'.format(artificial_file_name,file_name,np.mean(eq_arr),np.mean(onmi_arr)))
                # EQ_list.append(np.mean(eq_arr))
                # onmi_list.append(np.mean(onmi_arr))
                EQ_list.append(eq)
                onmi_list.append(onmi_val)
